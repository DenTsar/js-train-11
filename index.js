...
function customSet(arr) {
  ...
  for (const val of setN) {
    if (typeof val === "number") setN.delete(val);
  }
  return setN;
}
...
function removeDuplicatesInPlace(arr) {
  ...
  // const mySet = new Set(arr); гораздо проще
  return mySet;
}
...
function getDifference(set1, set2) {
  ...
  for (const val of set1) {
    if (!set2.has(val)) differenceSet.add(val);
  }
...
function getIntersection(arr1, arr2) {
  ...
  for (const val of set1) {
    if (set2.has(val)) intersectionSet.add(val);
  }
...
function sumNumbers(set) {
  ...
  let sum = 0;
  set.forEach((elem) => (typeof elem === "number" ? (sum = sum + elem) : elem));
  return sum;
}
